...
#include "FwdSplitCHOrderParam.h"

registerMooseObject("PhaseFieldApp", FwdSplitCHOrderParam);

template <>
InputParameters
validParams<FwdSplitCHOrderParam>()
{
...
FwdSplitCHOrderParam::FwdSplitCHOrderParam(const InputParameters & parameters)
...
void
FwdSplitCHOrderParam::initialSetup()
{
...
RealGradient
FwdSplitCHOrderParam::get_dM_dgradvar() // This function must be called in computeQp* functions
{
...
RealGradient
FwdSplitCHOrderParam::get_dM_dgradcvar(unsigned int icvar) // This function must be called in computeQp* functions
{
...
Real
FwdSplitCHOrderParam::computeQpResidual()
{
  if(fabs(sqrt(_grad_mu[_qp] * _grad_mu[_qp])) < 1e-20) {
    return 0.0;
  }else{
...
Real
FwdSplitCHOrderParam::computeQpJacobian()
{
...
Real
FwdSplitCHOrderParam::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real returnval = 0.0;
